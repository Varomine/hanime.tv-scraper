...
// .wrangler/tmp/bundle-JJ1TuE/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-JJ1TuE/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
// .wrangler/tmp/bundle-JJ1TuE/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
var handshakeSecret = "htv-insecure-handshake-v1";
var aadSecret = "htv-insecure-v1";
...
async function getCryptoKey(usages) {
...
async function encryptPayload(payload) {
  ...
  const encrypted = await crypto.subtle.encrypt(
    { name: "AES-GCM", iv, additionalData: encoder.encode(aadSecret), tagLength: 128 },
    key,
...
async function fetchSearchIndex(fetchFn) {
  const now = Date.now();
  ...
  const allVideos = Array.isArray(searchData) ? searchData : Array.isArray(searchData?.data) ? searchData.data : Object.values(searchData);
  cachedIndex = allVideos;
  cacheTime = now;
  return allVideos;
}
__name(fetchSearchIndex, "fetchSearchIndex");
async function getSearchIndex(fetchFn) {
  const now = Date.now();
  if (cachedIndex && now - cacheTime < 36e5) {
    return cachedIndex;
  }
...
__name(getSearchIndex, "getSearchIndex");
...
async function handleVideoRequest(slug) {
  ...
  try {
    ...
    const videoEntry = allVideos.find((v) => v.slug === slug);
    if (!videoEntry) {
    ...
    const payload = {
      timestamp_unix: Math.floor(Date.now() / 1e3),
    ...
    const franchiseVideos = brandName ? allVideos.filter((v) => v.brand === brandName && v.id !== videoEntry.id) : [];
    return jsonResponse({
      success: true,
      video: {
        id: videoEntry.id,
      ...
      franchise: {
        ...
        videos: franchiseVideos.slice(0, 10).map((v) => ({
          id: v.id,
          name: v.name,
          slug: v.slug,
...
async function handleLandingRequest() {
  try {
    const allVideos = await getSearchIndex(fetch);
    const trendingList = [...allVideos].sort((a, b) => (b.views || 0) - (a.views || 0)).slice(0, 24);
    const newReleasesList = [...allVideos].sort((a, b) => {
      const dateA = a.released_at ? new Date(a.released_at).getTime() : 0;
      const dateB = b.released_at ? new Date(b.released_at).getTime() : 0;
      return dateB - dateA;
    }).slice(0, 24);
    const mostLikedList = [...allVideos].sort((a, b) => (b.likes || 0) - (a.likes || 0)).slice(0, 24);
    const responseSections = [
      {
        title: "Trending",
        videos: trendingList.map((v) => ({
          id: v.id,
          name: v.name,
          slug: v.slug,
          brand: v.brand,
...
async function handleSearchRequest(query, page) {
  try {
    ...
    let filtered = allVideos.filter((v) => {
      return v.name && v.name.toLowerCase().includes(queryLower) || v.search_titles && v.search_titles.toLowerCase().includes(queryLower) || v.brand && v.brand.toLowerCase().includes(queryLower) || v.slug && v.slug.toLowerCase().includes(queryLower) || v.tags && v.tags.some((t) => t.toLowerCase().includes(queryLower));
    });
    filtered.sort((a, b) => {
    ...
    return jsonResponse({
      ...
      nbHits: totalHits,
      hitsPerPage,
      results: paginatedResults.map((h) => ({
        id: h.id,
        name: h.name,
        slug: h.slug,
        brand: h.brand,
