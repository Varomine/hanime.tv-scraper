...
// .wrangler/tmp/bundle-CuYsK6/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-CuYsK6/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
// .wrangler/tmp/bundle-CuYsK6/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
var src_default = {
  async fetch(request) {
    ...
    if (url.pathname === "/api/sig") {
      try {
        const sigs = await getSignature(fetch);
        return jsonResponse({ success: true, ...sigs });
      } catch (e) {
...
async function handleVideoRequest(slug) {
  ...
  try {
    ...
    const apiUrl = `https://guest.freeanimehentai.net/api/v8/video?id=${slug}`;
    ...
    const manifestUrl = `https://guest.freeanimehentai.net/api/v8/guest/videos/${videoId}/manifest`;
    ...
    if (manifestRes.ok) {
      ...
      const fallbackUrl = `https://h.freeanimehentai.net/api/v8/guest/videos/${videoId}/manifest`;
      ...
      if (!fallbackRes.ok) {
        return jsonResponse({ error: `Failed to fetch streaming manifest from CDN. Status: ${fallbackRes.status}` }, 500);
      }
      manifestData = await fallbackRes.json();
    }
...
async function handleLandingRequest() {
  ...
  try {
    ...
    const landingRes = await fetch("https://guest.freeanimehentai.net/api/v8/landing", {
      headers: {
        "Accept": "application/json",
        "Origin": "https://hanime.tv",
        "Referer": "https://hanime.tv/",
